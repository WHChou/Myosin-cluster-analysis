...
%       	will be used. If most pixels are 0, discard bottom 20% instead
...
% 		f(:,3): integrated brightness of the features. ("mass", from bpass)
...
%       f(:,6): integrated brightness of the features from raw image
%  RESTRICTIONS:
...
for i=1:nmax,    
    ...
    if Ihalo>0
        ...
        suba(:,:,i) = fracshift( double(newIm(fix(yl(i)):fix(yh(i)),fix(xl(i)):fix(xh(i)))), -xc(i) , -yc(i) );
        m2(i) = sum(sum(( suba(:,:,i).*mask )));%/length(find(mask));             % mass
        %newIm(fix(yl(i)):fix(yh(i)),fix(xl(i)):fix(xh(i))) = 0;
    end
    %%%%%%%%%%%%%
